<html>
    <head>
        ...
        <title>{{$edit?__("agent.Template").__("agent.Change") : __("agent.Template").__("agent.Add")}}</title>
        @stack('css')
                        ...
                        <div class="card">
                            <div class="card-header border-0" id="headingOne">
                                <div class="row align-items-center box">
                                    <div class="col-8">
                                        <h4 class="mb-0 title">{{$edit?__("agent.Template").__("agent.Change") : __("agent.Template").__("agent.Add")}}</h4>
                                    </div>
                                </div>
                            </div>
                            <div class="card-body row">
                                {{--<form action="{{$edit?argon_route('msgtemp.update', ['id' => $msgtemp->id, 'edit'=>true]) : argon_route('msgtemp.store',['edit'=>false])}}" id="user-form" method="POST" >--}}
                                <form action="{{$edit?argon_route('msgtemp.update', ['id' => $msgtemp->id, 'edit'=>true]) : argon_route('msgtemp.update',['id' => '', 'edit'=>false])}}" id="user-form" method="POST" >
                                    @csrf
                                    @if ($errors->count()>0)
                                    ...
                                    <div class="col-md-12 col-12">                                
                                        @include('notices.msgtemp.partials.base', ['edit' => $edit])
                                        <div class="text-center">
                                            <button type="submit" class="btn btn-primary mt-4 col-6" id="submitbtn">{{$edit?__("agent.Template").__("agent.Change") : __("agent.Template").__("agent.Add")}}</button>
                                        </div>
                                    </div>
                                </form>
                            </div>
...
<script type="text/javascript">
    ...
    if (document.getElementById('content')) {
        var editor = new Quill('#quillArea', {
            theme: 'snow'
        });
