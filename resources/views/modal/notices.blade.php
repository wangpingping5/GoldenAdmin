<html>
    <head>
        ...
        <title>{{$edit == 1?__("Notices").__("agent.Change") : __("Notices").__("agent.Add")}}</title>
        @stack('css')
                        ...
                        <div class="card">
                            <div class="card-header border-0" id="headingOne">
                                <div class="row align-items-center box">
                                    <div class="col-8">
                                        <h4 class="mb-0 title">{{$edit == 1?__("Notices").__("agent.Change") : __("Notices").__("agent.Add")}}</h4>
                                    </div>
                                </div>
                            </div>
                            <div class="card-body row">
                                <form action="{{$edit?argon_route('notices.update', ['id' => $notice->id, 'edit'=>true]) : argon_route('notices.store',['id' => '', 'edit'=>false])}}" id="user-form" method="POST" >
                                    @csrf
                                    @if ($errors->count()>0)
                                    ...
                                    <div class="col-md-12 col-12">                                
                                        @include('notices.partials.base', ['edit' => $edit])
                                        <div class="text-center">
                                            <button type="submit" class="btn btn-primary mt-4 col-6" id="submitbtn">{{$edit == 1?__("Notices").__("agent.Change") : __("Notices").__("agent.Add")}}</button>
                                        </div>
                                    </div>
                                </form>
                            </div>
