...
namespace App\Models
{
    class Game extends \Illuminate\Database\Eloquent\Model
    {
        ...
        public function bonus_bank()
        {
            ...
            $bank = \App\Models\BonusBank::lockforUpdate()->where(['master_id' => $master->id, 'game_id' => $this->original_id])->first();
            if (!$bank)
            {
                // $bank = \App\Models\BonusBank::lockforUpdate()->where(['master_id' => $master->id, 'game_id' => 0])->first();
                // if (!$bank){
                    $bank = \App\Models\BonusBank::create(['master_id' => $master->id, 'game_id' => $this->original_id]);
                // }
            }
            return $bank;
        }
        ...
        {
            return $this->hasMany('App\Models\GameCategory', 'game_id');
        }
